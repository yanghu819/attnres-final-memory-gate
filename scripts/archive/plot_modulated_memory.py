...
RAW = ROOT / 'results/raw/modulated_memory_results.tsv'
FIGS = ROOT / 'results/figs'
RUNS = ROOT / 'runs/modulated_memory'
...
COLORS = {
    ...
    'modkv_kv005': '#2ca02c',
    'modkv_kv010': '#d62728',
    'modqkv_q001_kv002': '#9467bd',
    'modqkv_q002_kv005': '#8c564b',
}
LABELS = {
    'strong_baseline': 'AttnRes block2',
    'blend_ngram_ref': 'Blend unigram+bigram',
    'modkv_kv002': 'ModKV 0.02',
    'modkv_kv005': 'ModKV 0.05',
    'modkv_kv010': 'ModKV 0.10',
    'modqkv_q001_kv002': 'ModQKV q0.01 kv0.02',
    'modqkv_q002_kv005': 'ModQKV q0.02 kv0.05',
}
...
def plot_loss_curves(df: pd.DataFrame) -> None:
    ...
    ax.set_title('Organic Memory-Conditioned AttnRes: loss curves')
    ax.legend(frameon=False)
    ax.grid(alpha=0.2)
    fig.tight_layout()
    fig.savefig(FIGS / 'fig_modulated_memory_loss_curves.png', dpi=200)
    plt.close(fig)
...
def plot_summary(df: pd.DataFrame) -> None:
    ...
    fig.tight_layout()
    fig.savefig(FIGS / 'fig_modulated_memory_summary.png', dpi=200)
    plt.close(fig)
