...
cd "$(dirname "$0")/.."
mkdir -p results/raw runs/golf_final_lightblend_refine

RESULTS=results/raw/golf_final_lightblend_refine_results.tsv
LOG_DIR=runs/golf_final_lightblend_refine
...
Path("results/raw/golf_final_lightblend_refine_results.tsv").write_text(
    "\t".join(
        [
            "config",
            "seed",
...
run_baseline() {
  local seed="$1"
  local log_path="$LOG_DIR/golf_attnres_tiny_seed${seed}.log"
  echo "== golf_attnres_tiny_seed${seed} =="
  AUTORESEARCH_LOG_PATH="$log_path" \
  AUTORESEARCH_SEED="$seed" \
  AUTORESEARCH_REFERENCE_LR="0.0012" \
  AUTORESEARCH_MAX_STEPS=64 \
  ...
  "$PYTHON_BIN" -m uv run python -m autoresearch_attnres_project.cli train --preset attnres_block2 \
    > "$log_path" 2>&1
  parse_log "golf_attnres_tiny" "$seed" "$log_path"
}

for seed in 45 46; do
  run_baseline "$seed"
  run_case "golf_final_lightblend_r16_b8192_c085" "$seed" 16 8192 0.85
  run_case "golf_final_lightblend_r16_b16384_c085" "$seed" 16 16384 0.85
  run_case "golf_final_lightblend_r24_b16384_c085" "$seed" 24 16384 0.85
  run_case "golf_final_lightblend_r32_b16384_c085" "$seed" 32 16384 0.85
  run_case "golf_final_lightblend_r24_b16384_c080" "$seed" 24 16384 0.80
done
