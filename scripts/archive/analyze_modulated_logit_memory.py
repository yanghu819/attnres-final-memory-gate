...
OUT = ROOT / 'results/modulated_logit_memory_analysis.txt'
TSV = ROOT / 'results/modulated_logit_memory_results.tsv'
...
def main() -> None:
    ...
    for config in ORDER:
        ...
        rows.append({
            ...
            'mean_logit_delta_norm': float(pd.to_numeric(sub['attnres_final_memory_mod_logit_delta_norm'], errors='coerce').mean()),
        })
    summary = pd.DataFrame(rows)
    summary.to_csv(TSV, sep='\t', index=False)
    ref = float(summary.loc[summary['config'] == 'blend_ngram_ref', 'mean_val_bpb'].iloc[0]) if (summary['config'] == 'blend_ngram_ref').any() else float('nan')
    ...
    if pd.notna(ref):
        lines.append(f"delta_vs_blend\t{best_row['mean_val_bpb'] - ref:.6f}")
    for row in summary.itertuples(index=False):
        lines.append(
            f"{row.config}\tmean={row.mean_val_bpb:.6f}\tstd={row.std_val_bpb:.6f}\tlatest={row.mean_final_latest:.6f}\tx0={row.mean_final_x0:.6f}\tentropy={row.mean_final_entropy:.6f}\tq={row.mean_q_delta_norm:.6f}\tk={row.mean_k_delta_norm:.6f}\tv={row.mean_v_delta_norm:.6f}\tlogit={row.mean_logit_delta_norm:.6f}"
        )
    OUT.write_text('\n'.join(lines) + '\n')
