...
cd "$(dirname "$0")/.."
mkdir -p results/raw runs/golf_final_lightblend_verify

RESULTS=results/raw/golf_final_lightblend_verify_results.tsv
LOG_DIR=runs/golf_final_lightblend_verify
...
Path("results/raw/golf_final_lightblend_verify_results.tsv").write_text(
    "\t".join(
        [
            "config",
            "seed",
