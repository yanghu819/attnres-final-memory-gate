...
RESULTS = ROOT / 'results' / 'raw' / 'input_memory_ablation_results.tsv'
RUNS = ROOT / 'runs' / 'input_memory_ablation'
OUT = ROOT / 'results' / 'figs'
OUT.mkdir(parents=True, exist_ok=True)

FAMILIES = {
    'strong_baseline': ('AttnRes Block2', '#1f3b4d'),
    'blend_ngram_ref': ('Final Blend (Uni+Bi)', '#2a9d8f'),
    'input_bigram_hash': ('Input BigramHash', '#c8553d'),
    'input_bigram_hash_smear': ('Input BigramHash + SmearGate', '#7a306c'),
    'input_bigram_trigram_hash': ('Input Bigram+Trigram', '#3a86ff'),
}
...
def plot_curves(loaded):
    fig, ax = plt.subplots(figsize=(11.6, 6.6), dpi=180)
    ...
    ax.set_title('Input Memory Ablation')
    ax.set_xlabel('Step')
    ax.set_ylabel('EMA train loss')
    ax.grid(True, alpha=0.2)
    ax.legend(frameon=False, fontsize=8)
    fig.tight_layout()
    fig.savefig(OUT / 'fig_input_memory_ablation_loss_curves.png')
    plt.close(fig)
...
def plot_val(loaded):
    ...
    ax.set_title('Input Memory Ablation: val_bpb')
    ax.grid(True, axis='y', alpha=0.2)
    fig.tight_layout()
    fig.savefig(OUT / 'fig_input_memory_ablation_valbpb.png')
    plt.close(fig)
...
def export_summary(loaded):
    ...
    (ROOT / 'results' / 'input_memory_ablation_results.tsv').write_text('\n'.join(lines) + '\n')
