...
RESULTS = ROOT / "results" / "raw" / "memory_sources_results.tsv"
RUNS = ROOT / "runs" / "memory_sources"
...
FAMILIES = {
    ...
    "static_factorized_r32_s025_c085": ("Factorized r=32", "#00798c"),
    "static_factorized_r64_s025_c085": ("Factorized r=64", "#edae49"),
    "static_factorized_r128_s025_c085": ("Factorized r=128", "#6a4c93"),
    "static_tied_s025_c085": ("Tied Embedding", "#8f2d56"),
}
...
def plot_curves(loaded):
    ...
    ax.set_title("Final Memory Source Ablation")
    ax.set_xlabel("Step")
    ax.set_ylabel("EMA train loss")
    ax.grid(True, alpha=0.2)
    ax.legend(frameon=False, fontsize=8)
    fig.tight_layout()
    fig.savefig(OUT / "fig_memory_sources_loss_curves.png")
    plt.close(fig)
...
def plot_val(loaded):
    ...
    ax.set_title("Final Memory Source Ablation: val_bpb")
    ax.grid(True, axis="y", alpha=0.2)
    fig.tight_layout()
    fig.savefig(OUT / "fig_memory_sources_valbpb.png")
    plt.close(fig)
...
def export_summary(loaded):
    ...
    (ROOT / "results" / "memory_sources_results.tsv").write_text("\n".join(lines) + "\n")
