...
cd "$(dirname "$0")/.."
mkdir -p results/raw runs/static_gate_refine
RESULTS=results/raw/static_gate_refine_results.tsv
LOG_DIR=runs/static_gate_refine
...
Path('results/raw/static_gate_refine_results.tsv').write_text('\t'.join([
  'config','seed','reference_lr','value_scale','gate_cap','gate_bias_init',
  'val_bpb','num_steps','attnres_final_memory_gate','attnres_final_memory_delta_norm','peak_vram_mb'
]) + '\n')
PY

parse_log() {
  local name="$1" seed="$2" lr="$3" value_scale="$4" gate_cap="$5" gate_bias_init="$6" log_path="$7"
...
print('\t'.join([
  ${name@Q}, ${seed@Q}, ${lr@Q}, ${value_scale@Q}, ${gate_cap@Q}, ${gate_bias_init@Q},
...
run_control() {
  ...
  parse_log "$name" "$seed" "$lr" "0.0" "0.0" "0.0" "$log_path"
}
...
run_projected_cap() {
  ...
  parse_log "$name" "$seed" "$lr" "0.3125" "0.85" "0.0" "$log_path"
}

run_static() {
  local name="$1" seed="$2" value_scale="$3" gate_cap="$4" gate_bias_init="$5"
  local lr="0.0012"
  local log_path="$LOG_DIR/${name}_seed${seed}.log"
  echo "== ${name}_seed${seed} =="
  AUTORESEARCH_LOG_PATH="$log_path" \
  ...
  parse_log "$name" "$seed" "$lr" "$value_scale" "$gate_cap" "$gate_bias_init" "$log_path"
}

for seed in 45 46 47 48; do
  run_control strong_baseline "$seed"
  run_projected_cap projected_cap085 "$seed"
  run_static static_c080_s03125 "$seed" 0.3125 0.80 4.0
  run_static static_c085_s02500 "$seed" 0.2500 0.85 4.0
  run_static static_c085_s03125 "$seed" 0.3125 0.85 4.0
  run_static static_c085_s03750 "$seed" 0.3750 0.85 4.0
  run_static static_c090_s03125 "$seed" 0.3125 0.90 4.0
done
