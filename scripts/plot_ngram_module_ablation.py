...
RESULTS = ROOT / 'results' / 'raw' / 'ngram_module_ablation_results.tsv'
RUNS = ROOT / 'runs' / 'ngram_module_ablation'
OUT = ROOT / 'results' / 'figs'
OUT.mkdir(parents=True, exist_ok=True)

FAMILIES = {
    'strong_baseline': ('AttnRes Block2', '#1f3b4d'),
    'unigram_only_matched': ('Unigram only (cap 0.85)', '#8f2d56'),
    'unigram_only_best': ('Unigram only (best)', '#d1495b'),
    'bigram_only_4x1m': ('Bigram only (4x1M)', '#00798c'),
    'unigram_plus_bigram_4x1m': ('Unigram + Bigram (4x1M)', '#2a9d8f'),
}
...
def plot_curves(loaded):
    ...
    ax.set_title('N-gram Module Ablation')
    ax.set_xlabel('Step')
    ax.set_ylabel('EMA train loss')
    ax.grid(True, alpha=0.2)
    ax.legend(frameon=False, fontsize=8)
    fig.tight_layout()
    fig.savefig(OUT / 'fig_ngram_module_ablation_loss_curves.png')
    plt.close(fig)
...
def plot_val(loaded):
    ...
    ax.set_title('N-gram Module Ablation: val_bpb')
    ax.grid(True, axis='y', alpha=0.2)
    fig.tight_layout()
    fig.savefig(OUT / 'fig_ngram_module_ablation_valbpb.png')
    plt.close(fig)
...
def export_summary(loaded):
    ...
    (ROOT / 'results' / 'ngram_module_ablation_results.tsv').write_text('\n'.join(lines) + '\n')
