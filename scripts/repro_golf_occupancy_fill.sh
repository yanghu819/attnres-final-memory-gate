...
cd "$(dirname "$0")/.."
mkdir -p results/raw runs/golf_occupancy_fill

RESULTS=results/raw/golf_occupancy_fill_results.tsv
LOG_DIR=runs/golf_occupancy_fill
DEADLINE_EPOCH="${DEADLINE_EPOCH:?DEADLINE_EPOCH required}"
...
Path("results/raw/golf_occupancy_fill_results.tsv").write_text(
    "\t".join(
        [
            "config",
            "seed",
...
while [ "$(date +%s)" -lt "$DEADLINE_EPOCH" ]; do
  if (( seed % 2 == 0 )); then
    run_case "golf_final_lightblend_r96_b65536_c090_fill" "$seed" 96 65536 0.90
  else
    run_case "golf_final_lightblend_r64_b32768_c095_fill" "$seed" 64 32768 0.95
  fi
  seed=$((seed + 1))
done
