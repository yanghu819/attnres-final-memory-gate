...
cd "$(dirname "$0")/.."
mkdir -p results/raw runs/lightweight_final_blend_verify results/figs
RESULTS=results/raw/lightweight_final_blend_verify_results.tsv
LOG_DIR=runs/lightweight_final_blend_verify
LOCK_DIR=runs/lightweight_final_blend_verify.lock
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "lightweight_final_blend_verify is already running: $LOCK_DIR" >&2
  exit 1
fi
...
Path('results/raw/lightweight_final_blend_verify_results.tsv').write_text('\t'.join([
...
PYTHONPATH=src "$PYTHON_BIN" scripts/analyze_lightweight_final_blend_verify.py
