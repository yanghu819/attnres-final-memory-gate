...
RESULTS = ROOT / 'results' / 'raw' / 'bigram_80pct_refine_results.tsv'
RUNS = ROOT / 'runs' / 'bigram_80pct_refine'
OUT = ROOT / 'results' / 'figs'
OUT.mkdir(parents=True, exist_ok=True)

FAMILIES = {
    'current_ref_1g_1b_16k': ('1 bank x 16k', '#1f3b4d'),
    'ref_4x1m_g1': ('4 banks x 1M (g=1)', '#d1495b'),
    'ref_4x1m_g4': ('4 banks x 1M (g=4)', '#00798c'),
    'bigram_5x1m_g1': ('5 banks x 1M', '#edae49'),
    'bigram_6x1m_g1': ('6 banks x 1M', '#6a4c93'),
    'bigram_7x1m_g1': ('7 banks x 1M', '#2a9d8f'),
    'bigram_6x1m_g4': ('6 banks x 1M (g=4)', '#8f2d56'),
}
...
def load_group(df: pd.DataFrame, family: str):
    ...
    return sub, [RUNS / f"{name}_g{groups}_bk{banks}_b{buckets}_seed{seed}.log" for name, groups, banks, buckets, seed in zip(sub['config'], sub['groups'], sub['banks'], sub['bigram_buckets'], sub['seed'])]
...
def plot_curves(loaded):
    ...
    for family, (label, color) in FAMILIES.items():
        ...
        ax.plot(merged['step'], mean, color=color, linewidth=2.1, label=f"{label} (n={used})")
        ax.fill_between(merged['step'], mean - std, mean + std, color=color, alpha=0.12)
    ax.set_title('Bigram Memory Scaling Near 80% VRAM')
    ax.set_xlabel('Step')
    ax.set_ylabel('EMA train loss')
    ax.grid(True, alpha=0.2)
    ax.legend(frameon=False, fontsize=8, ncol=2)
    fig.tight_layout()
    fig.savefig(OUT / 'fig_bigram_80pct_refine_loss_curves.png')
    plt.close(fig)
...
def plot_val(loaded):
    ...
    ax.set_title('Bigram Memory Scaling Near 80% VRAM: val_bpb')
    ax.grid(True, axis='y', alpha=0.2)
    fig.tight_layout()
    fig.savefig(OUT / 'fig_bigram_80pct_refine_valbpb.png')
    plt.close(fig)
...
def export_summary(loaded):
    ...
    (ROOT / 'results' / 'bigram_80pct_refine_results.tsv').write_text('\n'.join(lines) + '\n')
