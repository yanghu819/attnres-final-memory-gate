...
cd "$(dirname "$0")/.."
mkdir -p results/raw runs/modqkv_local_grid
RESULTS=results/raw/modqkv_local_grid_results.tsv
LOG_DIR=runs/modqkv_local_grid
...
Path('results/raw/modqkv_local_grid_results.tsv').write_text('\t'.join([
...
for seed in 45 46 47 48; do
  run_control "$seed"
  run_blend_ref "$seed"
  for q in 0.015 0.020 0.025; do
    for k in 0.04 0.05 0.06; do
      for v in 0.04 0.05 0.06; do
        qtag="${q/./}"
        ktag="${k/./}"
        vtag="${v/./}"
        run_variant "modqkv_q${qtag}_k${ktag}_v${vtag}" "$seed" "$q" "$k" "$v"
      done
    done
  done
done
