...
RESULTS = ROOT / "results" / "raw" / "simple_gate_results.tsv"
RUNS = ROOT / "runs" / "simple_gate"
...
GROUPS = {
    ...
    "final_memory_static": ("Final Memory Static", "#d1495b"),
    "final_memory_dynamic": ("Final Memory Dynamic 0.85", "#edae49"),
    "final_memory_dynamic_cap090": ("Final Memory Dynamic 0.90", "#6a4c93"),
}
...
def plot_curves(loaded):
    ...
    ax.set_title("Simple Final Memory Gate Ablation")
    ax.set_xlabel("Step")
    ax.set_ylabel("EMA train loss")
    ax.grid(True, alpha=0.2)
    ax.legend(frameon=False, fontsize=9)
    fig.tight_layout()
    fig.savefig(OUT / "fig_simple_gate_loss_curves.png")
    plt.close(fig)
...
def plot_val(loaded):
    ...
    ax.set_title("Simple Final Memory Gate: val_bpb")
    ax.grid(True, axis="y", alpha=0.2)
    fig.tight_layout()
    fig.savefig(OUT / "fig_simple_gate_valbpb.png")
    plt.close(fig)
...
def export_summary(loaded):
    ...
    (ROOT / "results" / "simple_gate_results.tsv").write_text("\n".join(lines) + "\n")
