...
RESULTS = ROOT / "results" / "raw" / "grouped_final_memory_results.tsv"
RUNS = ROOT / "runs" / "grouped_final_memory"
OUT = ROOT / "results" / "figs"
OUT.mkdir(parents=True, exist_ok=True)

GROUPS = {
    "strong_baseline": ("AttnRes Block2", "#1f3b4d"),
    "static_g1_s025_c085": ("Static G=1 s=0.25", "#d1495b"),
    "static_g2_s025_c085": ("Static G=2 s=0.25", "#00798c"),
    "static_g4_s025_c085": ("Static G=4 s=0.25", "#edae49"),
    "static_g2_s03125_c085": ("Static G=2 s=0.3125", "#6a4c93"),
    "static_g4_s03125_c085": ("Static G=4 s=0.3125", "#8f2d56"),
}
...
def plot_curves(loaded):
    ...
    ax.set_title("Grouped Final Memory Gate Ablation")
    ax.set_xlabel("Step")
    ax.set_ylabel("EMA train loss")
    ax.grid(True, alpha=0.2)
    ax.legend(frameon=False, fontsize=8)
    fig.tight_layout()
    fig.savefig(OUT / "fig_grouped_final_memory_loss_curves.png")
    plt.close(fig)
...
def plot_val(loaded):
    ...
    ax.set_title("Grouped Final Memory Gate: val_bpb")
    ax.grid(True, axis="y", alpha=0.2)
    fig.tight_layout()
    fig.savefig(OUT / "fig_grouped_final_memory_valbpb.png")
    plt.close(fig)
...
def export_summary(loaded):
    ...
    (ROOT / "results" / "grouped_final_memory_results.tsv").write_text("\n".join(lines) + "\n")
