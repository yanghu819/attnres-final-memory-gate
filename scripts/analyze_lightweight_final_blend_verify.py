...
raw_path = root / 'results' / 'raw' / 'lightweight_final_blend_verify_results.tsv'
out_path = root / 'results' / 'lightweight_final_blend_verify_analysis.txt'
...
if 'blend_ngram_ref' in by_cfg:
    blend_mean = mean(r['val_bpb'] for r in by_cfg['blend_ngram_ref'])
...
for cfg, cfg_rows in sorted(by_cfg.items(), key=lambda kv: mean(r['val_bpb'] for r in kv[1])):
    ...
    if blend_mean is not None:
        line += f"\tdelta_vs_full_blend={mean(vals) - blend_mean:+.6f}"
    lines.append(line)
